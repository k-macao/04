...
**8.1 预测与验证登记（必做）**：把核心论点拆成可到期验证的经营、催化剂和估值预测。保留原始预测及时间戳，后续更新只追加结果和误差归因，不得改写原预测。

| 预测对象 | 基准值/截止日 | 预测区间或方向 | 验证日期 | 先行指标与阈值 | 上/下行失效条件 | 状态 |
|---|---|---|---|---|---|---|

英文报告使用等价字段：Forecast; Baseline/data cutoff; Range or direction; Validation date; Leading indicators and thresholds; Upside/downside invalidation; Status。
...
若存在上一版报告或到期预测，在本节先附**预测复盘表**。经营 KPI、催化剂路径、估值倍数和总回报分别评价；股价变化不得单独证明经营论点正确。

| 原预测及时间戳 | 实际结果 | 命中状态 | 误差归因 | 模型/概率回写 |
|---|---|---|---|---|
